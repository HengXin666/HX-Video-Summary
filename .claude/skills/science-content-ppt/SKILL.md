...
**风格适配要求（重要）：**
> 生成PPT时必须遵循以下风格规范，确保输出质量一致：
> 1. **图标库**：统一使用 Font Awesome 6.x（`https://cdnjs.cloudflare.com/ajax/libs/font-awesome/6.4.0/css/all.min.css`），不使用 Remixicon
> 2. **配色方案**：优先使用暖色系（橙色 `#FF6B35`、琥珀 `#F4A261`、珊瑚 `#E76F51` 为主色），搭配深色背景 `#0F0E17`、浅色文字 `#FFFFFE`、辅助灰 `#A7A9BE`；警示用红 `#E71D36`，成功用绿 `#2EC4B6`
> 3. **组件样式**：使用 `border-left` 装饰的卡片、`warn-box`（红边警示框）、`success-box`（绿边成功框）、`vs-row`（VS对比双栏）、`icon-text`（图标+文字行）、`grid-2`（双列网格）
> 4. **导航**：底部固定导航栏（上一页/下一页按钮+页码），支持键盘、点击、滚轮、触摸翻页
> 5. **页码**：左上角显示 `01 / 12` 格式的slide编号
> 6. **动画**：使用 `anim-item` class + `.visible` class 控制元素依次"缓入"出现；切换页面时通过克隆节点强制重置CSS动画
> 7. **装饰**：`bg-grid` 背景网格、`glow-orb` 光晕、`particles` 粒子效果、脉冲发光图标
> 8. **总结+奋斗方向**：最后两页必须有"核心洞察总结"和"日后奋斗方向"页面，奋斗方向需包含：以技术入行打基础、感知变化顺势为、不躺平持续拼
